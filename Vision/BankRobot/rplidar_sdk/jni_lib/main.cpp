...
#include "rplidar.h" //RPLIDAR standard sdk, all-in-one header#include "com_nb_robot_lidar_RplidarNative.h"#ifndef _countof#define _countof(_Array) (int)(sizeof(_Array) / sizeof(_Array[0]))#endif#ifdef _WIN32#include <Windows.h>#define delay(x)   ::Sleep(x)#else#include <unistd.h>static inline void delay(_word_size_t ms) {
	while (ms >= 1000) {
		usleep(1000 * 1000);
		ms -= 1000;
	};
...
JNIEXPORT jboolean JNICALL Java_com_nb_robot_lidar_RplidarNative_connect
  (JNIEnv *env, jobject obj, jlong driver_address, jstring port_path) {
	RPlidarDriver* driver = (RPlidarDriver*)driver_address;
	const char* native_port_path = env->GetStringUTFChars(port_path, JNI_FALSE);
	_u32 opt_com_baudrate = 115200;
	...
	env->ReleaseStringUTFChars(port_path, native_port_path);
	return is_ok;
}
...
(JNIEnv *env, jobject obj, jlong driver_address) {
	RPlidarDriver* driver = (RPlidarDriver*)driver_address;
...
jobject copyMeasurementNodeToJavaObject(JNIEnv* env,
	...
	printf("%s theta: %03.2f Dist: %08.2f Q: %d \n",
			(measurement_node.sync_quality & RPLIDAR_RESP_MEASUREMENT_SYNCBIT) ?
					"S " : "  ",
					angle,
					distance,
					sync_quality);

	jclass measurementNodeClass = env->FindClass(
			"com/nb/robot/lidar/MeasurementNode");
	jmethodID constructorMethod = env->GetMethodID(measurementNodeClass,
			"<init>", "(IFF)V");
	jobject measurementNodeClassObject = env->NewObject(measurementNodeClass,
			constructorMethod, sync_quality, angle, distance);
	return measurementNodeClassObject;
}

// Testing only.
void populateMeasurementNodesForTesting(rplidar_response_measurement_node_t *nodes) {
	size_t count = _countof(nodes);
	for (int pos = 0; pos < (int) count; ++pos) {
