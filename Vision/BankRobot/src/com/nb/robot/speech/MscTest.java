...
// Test Xunfei online Java SDK.
// NOTE: this is deprecated in favor of Linux offline SDK. See com.nb.robot.xf package. 
public class MscTest {
	private static final String APPID = "591cf535";
	...
	public void RecognizePcmfileByte() {
		...
		// 1 means mic input, -1 means audio file input.
...
//				"./iat_test.pcm");
		...
		for (int i = 0; i< 15 ; ++i) {
			...
			if (mIsEndOfSpeech && !recognizer.isListening()) {
				recognizer.startListening(recListener);
			}
			try {
				Thread.sleep(1000);
		...
		try {
			fis = new FileInputStream(new File("./test.wav"));
			if (0 == fis.available()) {
